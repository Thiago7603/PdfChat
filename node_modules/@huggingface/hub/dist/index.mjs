...
async function scanCachedRepo(repoPath) {
  ...
  const repoLastAccessed = blobStats.size > 0 ? Math.max(...[...blobStats.values()].map((stat2) => stat2.atimeMs)) : repoStats.atimeMs;
  const repoLastModified = blobStats.size > 0 ? Math.max(...[...blobStats.values()].map((stat2) => stat2.mtimeMs)) : repoStats.mtimeMs;
  return {
    id: {
      name: repoId,
      type: repoType
    ...
    size: [...blobStats.values()].reduce((sum, stat2) => sum + stat2.size, 0),
